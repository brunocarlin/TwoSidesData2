...
But be carefull you can't assign in pandas using the . you need to use the "[" operator
...
```{python}
df.to_csv(r.file_path_linux + '/finished_work.csv')
```

```{python}
pd.read_csv(r.file_path_linux + '/finished_work.csv').dtypes
```
...
```{python}
df.reset_index().to_feather(r.file_path_linux + '/sex_thesis_assignment.feather')
```
...
```{python}
pd.read_feather(r.file_path_linux + '/sex_thesis_assignment.feather').dtypes
```
...
```{python}
os.remove(r.file_path_linux + '/finished_work.csv')
#os.remove(r.file_path_linux + '/sex_thesis_assignment.feather')
```
